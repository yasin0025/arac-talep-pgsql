...
class UserProfile(models.Model):
    ...
    def __str__(self):
        return f"{self.user.username} - {self.rol}"# Araç Bilgileri
...
class EkKisiBilgisi(models.Model):
    ...
    user = models.ForeignKey(User, on_delete=models.SET_NULL, null=True, blank=True)  # 🔴 EKLENMİŞ OLMALI
...
class TalepOnay(models.Model):
    ...
    varis_saati = models.TimeField(null=True, blank=True)  # ⬅️ YENİ ALAN

    def __str__(self):
        return f"{self.talep.id} - {self.durum}"
